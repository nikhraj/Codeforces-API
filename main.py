...
@app.route("/contests/{contest_id}",methods=['GET'], response_model=contest_response)
async def contest_data(contest_id: int):

    results =  codeforces_contest_wrapper.parse_contest(CONTEST_LINK + str(contest_id))
    return results

@app.route("/problems/{contest_id}/{problem_id}",methods=['GET'], response_model=problem_response)
